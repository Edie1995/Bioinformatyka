...
mainSequence = comparedSequences(idx).madeSeq2;
matchSequences(idx) = struct('lastSeq',mainSequence);
for i = 1: length(comparedSequences)
    sequenceToCopare = comparedSequences(i).madeSeq2;
    if(~strcmp(mainSequence, sequenceToCopare))
        ...
        matchStruct = struct('lastSeq',lastSeq1);
